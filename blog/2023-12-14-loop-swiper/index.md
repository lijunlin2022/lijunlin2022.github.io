...
我们先思考一个问题。5 个元素、未播放的循环轮播应该怎么排？答案是 34012。
...
经过推理，我们得到答案 34012。但推理过程太麻烦了，有没有办法可以简化思考呢？
...
我们可以延续之前的思路，把 Item 1 放在中央，再按 12340 的顺序向右放置。放完 Item 1、Item 2 和 Item 3 后，Item 1 右边已经有 2 个元素了，接下来我们只需要把 Item 4 和 Item 0 平移到 Item 1 左边，就得到了 40123。
...
- 2 放在中央，按 23401 的顺序，把 3、4 放到 2 右边，再把 0、1 平移到 2 左边；
- 3 放在中央，按 34012 的顺序，把 4、0 放到 3 右边，再把 1、2 平移到 3 左边；
- 4 放在中央，按 40123 的顺序，把 0、1 放到 4 右边，再把 2、3 平移到 4 左边。
...
听完上述描述，想必你已经有一个模糊的思路，现在我们一起把思路转变为代码吧。先看 HTML 布局代码，我只保留布局关键部分，这样代码更精简。
...
这个布局是如何得到的呢？我们先看 translateX 均为 0% 的情况，Item 0 ~ Item 4 的 left 设置成 -400% ~ 0% 的情况。
...
效果图如下，可以看到所有 Item 都重叠在中央的位置：

![](./img/code-explain-init.png)

接着，我们给 Item 设置 translateX。把 Item 0 ~ Item 4 的 translateX 设置成 0% ~ 400%。
...
效果图如下，translateX 为正数时，元素会向右移动，正是我们在「轮播的原理」一节提到的「把 Item 按 01234 的顺序，从中央区域开始向右放置」。

![](./img/code-explain-first.png)

想必你已经猜到，下一步我们要把 Item 3 和 Item 4 平移到 Item 0 左边，也就是把 Item 3、Item 4 的 translateX 减去 500%（轮播的整体长度）。
...
到这里，我们已经解释好如何得到 HTML 初始布局了。

### 逻辑代码

下面我们来看 JavaScript 代码。之前的 HTML 代码，只是实现轮播图的初始顺序，接下来的 JavaScript 代码，则是实现播放前一个元素、播放后一个元素。
...
不难想到，把元素的 translateX 设置为正数，元素就会在中央元素左侧；把元素的 translateX 设置为负数，元素就会在中央元素右侧。
...
开始，我们通过 01234 转变为 34012 的例子，解释循环轮播的初始顺序如何设置。然后，我们用 HTML 代码，实现了 34012 顺序的轮播。

接着，我们用两种思路解释 JavaScript 代码：

- 第一种思路是先找到中央元素，然后往中央元素的右边放剩余元素，放完一半的剩余元素后，再把另外一半放到中央元素左边。
- 第二种思路是，先找到 translateX 为 0% 的元素，它就是中央元素。然后把其他元素的 translateX 限定在 -halfTotalWidth 和 halfTotalWidth 之间。

解释完代码后，我们给出了 JavaScript 不复制元素循环实现轮播的代码
