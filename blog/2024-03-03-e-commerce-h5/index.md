...
眼看着灵感就要烂在手里，我决定把两篇文章合为一篇，尽快与大家分享我的看法。当然，这些分析预测只是我的个人观点。如果你有不同的意见，欢迎在评论区讨论交流。

## 百亿补贴为什么用 H5?

我们首先看一张 F12 的控制台图，可以看到，百亿补贴拼多多技术栈是 H5，大概率是 React 写的 H5。
...
第一个原因，也是最基础的原因，就是 H5 技术已经成熟稳定，能够完整地实现功能。具体来说：

#### 浏览器兼容性不断提高

自 2008 年 HTML5 草案发布以来，至 2010 年乔布斯宣称 HTML5 将取代 Flash，再到 2022 年微软宣布 IE 浏览器正式退役，可以看到，主流浏览器对 HTML5、CSS3 和 JavaScript 的标准语法兼容性一直在持续改进。虽然 Safari（iOS 浏览器）的兼容性仍然备受诟病，但总体来说兼容成本已经变得可以接受。
...
- 从语法的角度看，自 React16.8 发布 Hooks，以及 Vue3 发布 Composition API 以来，两大框架语法基本稳定，不再有大的变化。前端开发者可以更加专注于业务逻辑，而无需过多担心框架本身的变动。
- 从未来发展方向看，React 目前致力于推广 React Server Component [^1]；Vue 则在尝试尝试着无 VDom 的 Vapor 方向，并计划利用 Rust 重写 Vite [^2]。这表明旧领域基本不再有大变动，两大框架已经正寻求新的发展领域，以继续保持竞争力。

#### 混合开发已经成熟

混合开发是指将原生开发（Android 和 iOS）和 Web 开发结合起来的一种技术。简而言之，它将 H5 嵌入到移动应用内部运行。近些年，业界对混合开发的优势和缺陷已经有清晰的认识，并针对其缺陷进行了相应的优化。具体来说：

- 混合开发的优势包括开发速度快、一套代码适配 Android 和 iOS，以及实现代码的热更新。这意味着开发者可以更快地开发出跨平台应用，并且能够及时对应用进行更新和修复。
...
前文我们已经提到「百亿补贴」选择 H5 技术栈的两个原因，现在我们来看第三个原因——H5 开发成本低。

「百亿补贴」的方式，是一个常住的 H5，搭配上多个流动的 H5。（「常住」和「流动」是我借鉴「常住人口」和「流动人口」造的词）

- 常住 H5 链接保持不变。站外投放的链接基本都是常住 H5 的，站内首页入口链接也是常住 H5 的，这样方便用户二次访问。
- 流动 H5 链接在常住 H5 的不同位置，点击后会从常住 H5 跳转。时间不同、用户不同、算法不同，都会让流动 H5 的链接发生变化。流动 H5 主要起流量分发的作用。
  
  具体来看，拼多多至少有三个流量的分发点，第一个是可点击的头图，第二个是列表上方的活动模块，第三个是右侧浮动的侧边栏。三者可以投放不同的链接。比如最近三个资源位就分别投放 3.8 女神节链接、新人链接和品牌链接。

![](./img/pdd-activity.png)

不难想到，每到一个节日、每换一个品牌，就有可能需要为这些节日、品牌定制化一套流动 H5 代码。如果使用其他技术栈，排期跟进通常会比较困难，但是使用 H5 技术，能够快速迭代并上线。

### H5 适合到处投放

在前文我们已经了解到，成熟的技术让 H5 完全可以实现「百亿补贴」功能。现在我们介绍另一个原因：H5 适合投放。

拼多多的崛起过程中，投放到其他 App 的链接功不可没。早期它通过微信等社交平台「砍一刀」的模式，低成本地吸引了大量用户。如今，它通过投放「百亿补贴」策略留住用户。

H5 的独特之处，在于它能够灵活地在多个平台上进行投放，其他技术栈很难有这样的灵活性。拼多多更是将这种灵活性发挥到极致。即使是今天，抖音、Bilibili 和小红书等其他 App 中，「百亿补贴」的广告也随处可见。

![](./img/pdd-advertisement.png)

更厉害的是，只要你有「百亿补贴」的链接，你甚至可以在微信、飞书、支付宝等地方直接查看拼多多的 H5 页面。
...
了解了「百亿补贴」选择 H5 的原因后，我们来看看电商巨头对 H5 未来发展的影响。我认为可能有三个影响：

### H5 数量膨胀，定制化要求苛刻

C 端用户黏性相对较低，换一个 App 的成本微不足道。再加上近年 C 端用户增长缓慢，企业已经难以容忍的用户丢失。因此当其他企业投放活动 H5 时，企业必须也要投放活动 H5，电商活动 H5 会变得越来越多。
...
因此，只有收益丰厚、技术示例雄厚的公司才更愿意使用 SSR。总不能向老板汇报结果时，告诉老板「虽然您亏钱了，但是 SSR 真的很酷」。
...
定制化要求苛刻，我猜测更多社区类 Shadcn UI 的 H5 组件库或许会兴起。现有的 H5 的组件库样式都太单一，即使是 Shadcn UI 这样的组件库，也很难满足国内 H5 的定制化开发需求。然而，Shadcn UI 的基本思路——「把源码下载到项目」，给定制化开发组件难复用的问题提供了很好的方向，因此，我认为类似 Shadcn 的 H5 组件库可能会逐渐兴起。
...
以及电商巨头可能对 H5 发展产生的三个影响：

- 数量膨胀，定制化要求苛刻
- SSR 比例增加，CSR 占据主流
- Monorepo 比例上上，类 Shadcn UI 组件库也许会兴起

总而言之，H5 开发会越来越专业，对程序员要求会越来越高。至于这种情况是好是坏，仁者见仁智者见智，欢饮大家在评论区沟通交流。
